...
//	private Bitmap whitePoint;
	...
	public void afterPlatformListGot() {
		...
		int gResId = cn.sharesdk.framework.utils.R.getBitmapRes(getContext(), "gray_point");
		int wResId = cn.sharesdk.framework.utils.R.getBitmapRes(getContext(), "white_point");
		for (int i = 0; i < pageCount; i++) {
			points[i] = new ImageView(context);
			points[i].setScaleType(ScaleType.CENTER_INSIDE);
			points[i].setImageResource(gResId);
		...
		points[curPage].setImageResource(wResId);
	}
	...
	private static class GridView extends LinearLayout {
		...
		private Bitmap getIcon(Platform plat) {
			...
			int resId = cn.sharesdk.framework.utils.R.getBitmapRes(getContext(), resName);
